...
app.get("/posts/:postPage", function(req, res){
	const requestedTitle = req.params.postPage
	var isTrue = false
	for(var i = 0; i < posts.length; i++){
		var pageTitle = lodash.kebabCase(posts[i].title)
		if(requestedTitle == pageTitle){
			res.render("post",{singleTitle: posts[i].title, singleBody: posts[i].body})
			isTrue = true
		}
	}if(isTrue = false){
	}
...
app.post("/compose", function(req, res){
	var pageTitle = lodash.kebabCase(req.body.postTitle)
	const postLink = "/posts/" + pageTitle
	var post = {
